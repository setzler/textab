...
#' @param position (character). If cspan > 1, `multicolumn{cspan}{position}{value}` will be used. For example, position=c("l","c","r") means that the second entry of `value` should be centered. Default is position = rep("c", length(value)).
...
#' @param dec (integer). Only relevant if `value` is numeric. Number of decimal places. If scalar, the same decimal will be used for each entry of `value`. If vector, must be the same length as `value`.
#' @param big.mark (character). Only relevant if `value` is numeric. Used as separator within large numbers. Default is "," which produces a comma (as in 1,201,390).
...
#' @param pvalues (numeric). Only relevant if `value` is numeric. If not NULL, must be numeric. If less than 0.1, a star "*" will be added to the end of each entry of `value`. If less than 0.05, a second star "*" will be appended. If less than 0.01, a third star "*" will be appended. If scalar, it will be used for all entries of `value`. If vector, must be the same length as `value`.
#' @param space (numeric). The number of points (pt) of vertical space to append to the end of the row. Default is 3, which creates 3pt of vertical space.
#' @export
TexRow <- function(value, cspan = rep(1, length(value)), position = rep("c", length(value)), surround = "%s", dec = 4, big.mark = ",", percentage = FALSE, dollar = FALSE, se = FALSE, pvalues = NULL, space = 0) {

  # if numeric, apply formatting rules to value. return character.
  if (class(value) %in% c("numeric")) {
    value <- tt_formatNum(x = value, dec = dec, big.mark = big.mark, percentage = percentage, dollar = dollar, se = se, pvalues = pvalues)
  }
