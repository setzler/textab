...
Let us define a temporary folder in which to save output:
...
```{r echo=TRUE, eval=FALSE, message=FALSE}
TexSave(tab = tt, positions = c("l","c","c"), filename = "example1", output_path = op)
```
...
```{r echo=TRUE, eval=FALSE, message=FALSE}
TexSave(tab = tt, positions = c("l","c","c"), filename = "example2", output_path = op, stand_alone = TRUE)
```
...
```{r echo=TRUE, eval=FALSE, message=FALSE}
TexSave(tab = tt, positions = c("l","c","c"), filename = "example3", output_path = op, 
        stand_alone = TRUE, compile_tex = TRUE)
```

The final command will produce a PDF similar to this: [here](https://github.com/setzler/textab/blob/main/inst/example3.pdf).
